...
export const LiveMixerInstrument: React.FC<LiveMixerInstrumentProps> = ({
  ...
  return (
    ...
    >
      <ContainerInstrument className="flex flex-col gap-6">
        {/* Header */}
        <ContainerInstrument className="flex items-center justify-between">
          ...
          <ContainerInstrument className="flex items-center gap-2">
            <ButtonInstrument 
              onClick={togglePlay}
              className={cn(
                "w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-md active:scale-95 p-0",
            ...
            {onRemove && (
              <ButtonInstrument 
                onClick={onRemove}
                className="w-12 h-12 rounded-2xl bg-va-black/5 text-va-black/20 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all p-0"
              >
        ...
        {/* Waveforms Container */}
        <ContainerInstrument className="relative bg-va-off-white/50 rounded-[24px] p-4 overflow-hidden border border-black/5">
          {/* Music Waveform (Background) */}
          <ContainerInstrument plain ref={musicWaveformRef} className="absolute inset-0 pointer-events-none opacity-50" />
          
          {/* Voice Waveform (Foreground) */}
          <ContainerInstrument plain ref={voiceWaveformRef} className="relative z-10" />
          ...
          <ContainerInstrument className="absolute top-2 left-4 flex gap-4 pointer-events-none">
            <ContainerInstrument className="flex items-center gap-1.5">
              <ContainerInstrument plain className="w-1.5 h-1.5 rounded-full bg-primary" />
              <TextInstrument as="span" className="text-[15px] font-black tracking-widest text-va-black/40">
                <VoiceglotText  translationKey="audio.review.voice" defaultText="Stem" />
              </TextInstrument>
            </ContainerInstrument>
            {musicUrl && (
              <ContainerInstrument className="flex items-center gap-1.5">
                <ContainerInstrument plain className="w-1.5 h-1.5 rounded-full bg-va-black/10" />
                <TextInstrument as="span" className="text-[15px] font-black tracking-widest text-va-black/40">
        ...
        {/* Controls */}
        {musicUrl && (
          <ContainerInstrument className="flex items-center gap-6 bg-va-off-white/30 p-4 rounded-2xl border border-black/5">
            <ContainerInstrument className="flex items-center gap-3 flex-1">
              <ContainerInstrument plain className="p-2 rounded-lg bg-white shadow-sm text-va-black/40">
              ...
              <ContainerInstrument className="flex-1 space-y-1">
                <ContainerInstrument className="flex justify-between items-center">
                  ...
                  <TextInstrument as="span" className="text-[15px] font-black text-primary">{Math.round(musicVolume * 100)}%</TextInstrument>
                </ContainerInstrument>
                <input 
                  type="range" 
                  min="0" 

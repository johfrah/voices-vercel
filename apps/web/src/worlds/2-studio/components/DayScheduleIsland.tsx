    ...
    <SectionInstrument className="relative">
      <ContainerInstrument className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
        ...
        <ContainerInstrument className="lg:col-span-5 space-y-8">
          ...
          {nextEdition && (
            <ContainerInstrument className="p-8 bg-va-black text-white rounded-[24px] shadow-aura-lg flex flex-col gap-6">
              <ContainerInstrument className="flex items-center gap-4">
                <ContainerInstrument className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary">
                  <Clock size={20} />
                </ContainerInstrument>
                <ContainerInstrument>
                  <TextInstrument className="text-[11px] font-bold tracking-[0.2em] uppercase text-white/30 mb-0.5">
                    <VoiceglotText translationKey="common.timing" defaultText="Timing" />
                  </TextInstrument>
              ...
              <ContainerInstrument className="flex items-center gap-4">
                ...
                <ContainerInstrument>
                  <TextInstrument className="text-[11px] font-bold tracking-[0.2em] uppercase text-white/30 mb-0.5">
                    <VoiceglotText translationKey="common.location" defaultText="Locatie" />
                  </TextInstrument>
                  <TextInstrument className="text-lg font-light tracking-tight">
        ...
        {/* Right: Timeline */}
        <ContainerInstrument className="lg:col-span-7 space-y-6 relative before:absolute before:left-[19px] before:top-8 before:bottom-8 before:w-px before:bg-va-black/[0.05]">
          {schedule.map((item: any, index: number) => (
            <ContainerInstrument key={index} className="relative pl-16 group">
              <ContainerInstrument className="absolute left-0 top-1.5 w-10 h-10 rounded-full bg-white border border-black/[0.05] shadow-sm flex items-center justify-center z-10 group-hover:border-primary/30 group-hover:shadow-aura-sm transition-all duration-500">
                <ContainerInstrument className="w-2 h-2 rounded-full bg-va-black/10 group-hover:bg-primary transition-colors duration-500" />
              </ContainerInstrument>
              
              <ContainerInstrument className="p-6 bg-white rounded-[20px] shadow-aura border border-black/[0.01] group-hover:shadow-aura-lg group-hover:-translate-y-1 transition-all duration-500">
                <ContainerInstrument className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">

    ...
    <ContainerInstrument className="grid grid-cols-1 md:grid-cols-12 gap-16 items-start mb-24">
      ...
      <ContainerInstrument className="md:col-span-6 space-y-10">
        ...
        <ContainerInstrument className="flex flex-col sm:flex-row gap-4">
          ...
          >
            <VoiceglotText translationKey="portfolio.footer.calculate_price" defaultText="Bereken mijn prijs" /> <ArrowRight size={16} />
          </ButtonInstrument>
          ...
          >
            <VoiceglotText translationKey="portfolio.footer.direct_contact" defaultText="Direct contact" />
          </ButtonInstrument>
        </ContainerInstrument>
      </ContainerInstrument>
      ...
      <ContainerInstrument className="md:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-12">
        <ContainerInstrument className="space-y-6">
          <HeadingInstrument level={4} className="text-[11px] font-black tracking-[0.2em] text-va-black/20 uppercase">
            <VoiceglotText translationKey="portfolio.footer.my_work" defaultText="Mijn Werk" />
          </HeadingInstrument>
          <ul className="space-y-3">
            {[
              { name: 'Voice-over Demos', href: '/demos', key: 'demos' },
              { name: 'Host & Reporter', href: '/host', key: 'host' },
              { name: 'Mijn Tarieven', href: '/tarieven', key: 'rates' },
              { name: 'Contact opnemen', href: '/contact', key: 'contact' },
            ].map((link) => (
              <li key={link.name}>
                <VoicesLinkInstrument href={link.href} className="text-[16px] font-light text-va-black/40 hover:text-va-black transition-colors">
                  <VoiceglotText translationKey={`portfolio.footer.nav.${link.key}`} defaultText={link.name} />
        ...
        <ContainerInstrument className="space-y-6">
          <HeadingInstrument level={4} className="text-[11px] font-black tracking-[0.2em] text-va-black/20 uppercase">
            <VoiceglotText translationKey="portfolio.footer.follow_actor" values={{ name: actorFirstName }} defaultText={`Volg ${actorFirstName}`} />
          </HeadingInstrument>
          {actorSocials.length > 0 ? (
            <ContainerInstrument className="flex flex-wrap gap-4">
              {actorSocials.map((social) => (
                <ButtonInstrument 
                  ...
                  size="none"
                  className="w-12 h-12 rounded-full bg-va-black/5 flex items-center justify-center hover:bg-va-black group transition-all duration-500 shadow-sm p-0"
                >
            ...
            <TextInstrument className="text-[14px] font-light text-va-black/20 italic">
              <VoiceglotText translationKey="portfolio.footer.no_socials" defaultText="Geen social media gekoppeld." />
            </TextInstrument>
          )}

          <ContainerInstrument className="pt-6 space-y-4">
            {actor?.phone && (
              <ButtonInstrument as="a" variant="plain" href={`tel:${actor.phone.replace(/\s+/g, '')}`} className="flex items-center gap-3 text-[15px] font-light text-va-black/60 hover:text-primary transition-colors group p-0 bg-transparent border-none">
            ...
            {actor?.email && (
              <ButtonInstrument as="a" variant="plain" href={`mailto:${actor.email}`} className="flex items-center gap-3 text-[15px] font-light text-va-black/60 hover:text-primary transition-colors group p-0 bg-transparent border-none">

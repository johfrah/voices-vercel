    ...
    <ContainerInstrument className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-12 gap-y-16 mb-24">
      ...
      <ContainerInstrument className="space-y-6 flex flex-col items-start">
        ...
        <ContainerInstrument className="space-y-3">
          <ButtonInstrument as="a" variant="plain" href={`mailto:${activeEmail}`} className="flex items-center gap-2 text-[15px] font-light text-white/40 hover:text-white transition-colors p-0 bg-transparent border-none">
            <Mail size={14} strokeWidth={1.5} />
            <TextInstrument as="span">{activeEmail}</TextInstrument>
          </ButtonInstrument>
          <ButtonInstrument as="a" variant="plain" href={`tel:${activePhone.replace(/\s+/g, '')}`} className="flex items-center gap-2 text-[15px] font-light text-white/40 hover:text-white transition-colors p-0 bg-transparent border-none">

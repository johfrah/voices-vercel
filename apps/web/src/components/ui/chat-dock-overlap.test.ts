...
describe('chat and casting dock overlap guards', () => {
  ...
  it('shows casting dock only with selection and closed chat', () => {
    expect(shouldShowCastingDock(1, false, false)).toBe(true);
    expect(shouldShowCastingDock(1, false, true)).toBe(false);
    expect(shouldShowCastingDock(0, false, false)).toBe(false);
    expect(shouldShowCastingDock(2, true, false)).toBe(false);
  });
});

          ...
          {sortedWorkshops.map((workshop) => {
            ...
            return (
              <ContainerInstrument plain key={workshop.id}>
                <VoicesLink href={`/studio/${workshop.slug}`} className="flex items-start gap-2.5 group">
      ...
      <ContainerInstrument className="space-y-6 flex flex-col items-start">
        ...
        <ContainerInstrument className="space-y-4 w-full">
          ...
          <ContainerInstrument className="pt-4 border-t border-black/5 space-y-3">
            <VoicesLink href={`mailto:${activeEmail}`} className="flex items-center gap-2 text-[14px] font-light text-va-black/60 hover:text-primary transition-colors">
              <Mail size={14} strokeWidth={1.5} />
              <TextInstrument as="span">{activeEmail}</TextInstrument>
            </VoicesLink>
            <VoicesLink href={`tel:${activePhone?.replace(/\s+/g, '') || ''}`} className="flex items-center gap-2 text-[14px] font-light text-va-black/60 hover:text-primary transition-colors">

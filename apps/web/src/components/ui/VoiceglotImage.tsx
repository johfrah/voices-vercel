...
export const VoiceglotImage: React.FC<VoiceglotImageProps> = ({ 
  ...
  // 🛡️ CHRIS-PROTOCOL: Market-Aware Logo Clipping (v2.28.71)
  // If we are on an international domain, we can hide the '.be' part of the animated SVG.
  const currentMarketCode = React.useMemo(() => {
    if (typeof window === 'undefined') return 'BE';
    try {
      return MarketManagerServer.getCurrentMarket(window.location.host, window.location.pathname).market_code;
    } catch (e) {
      return 'BE';
    }
  }, []);

  const showExtension = currentMarketCode === 'BE' || currentMarketCode === 'ADEMING';
  const viewBoxWidth = showExtension ? 3000 : 2500;
      ...
      >
        <div 
          ...
          dangerouslySetInnerHTML={{ 
            __html: `
              <svg viewBox="0 0 ${viewBoxWidth} 1179.28" class="w-full h-full" style="display: block; max-width: 100%; max-height: 100%;">
                <style>
                  .st0{fill:#FF2F8D;}
                  ...
                  /* 🛡️ CHRIS-PROTOCOL: Smooth Waveform Reset (v2.14.659) */
                  .bar { 
                  ...
                  svg:hover .dot { 
                    animation: dotBounce 2.80s ease-in-out infinite; 

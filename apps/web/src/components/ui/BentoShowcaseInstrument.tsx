...
import { TELEPHONY_CTA_HREF } from "./journey-cta-config";
      ...
      {detectedSector === 'it' || lastIntent === 'telephony' ? (
        <BentoCard span="xl" className="h-[500px] flex flex-col justify-between group overflow-hidden relative p-12 bg-va-black text-white border-none shadow-aura">
          ...
          <ContainerInstrument plain className="relative z-10 flex gap-4">
            <Link  href={TELEPHONY_CTA_HREF} className="va-btn-pro flex items-center gap-3 !bg-primary !text-va-black !rounded-[10px] !font-light !tracking-widest !"><VoiceglotText  translationKey="home.showcase.telephony.cta" defaultText="Configureer je keuzemenu" /><ArrowRight size={18} strokeWidth={1.5} /></Link>
          </ContainerInstrument>
        </BentoCard>
      ) : (

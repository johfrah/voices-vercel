...
import { PrivateVoiceBridge } from '@/lib/services/private-voice-bridge';
import { cn } from '@/lib/utils';

export default function JohfraiStudioPageContent() {
  ...
  const checkStatus = async () => {
    try {
      const isOnline = await PrivateVoiceBridge.checkHealth();
  ...
  const handleGenerate = async () => {
    ...
    try {
      const audioData = await PrivateVoiceBridge.generateAudio(text);
      if (audioData) {
  ...
  const handleSendMail = async () => {
    ...
    try {
      ...
      const targetEmail = 'johfrah' + '@' + 'voices' + '.be';
      ...
      if (result.success) {
        alert(`Audio succesvol verzonden naar ${targetEmail}! 🎙️📧`);
      } else {

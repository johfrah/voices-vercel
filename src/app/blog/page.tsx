...
export default function BlogPage() {
  ...
  const fallbackArticles = [
    ...
    },
    {
      id: 'creo',
      slug: 'story-creo',
      title: 'CREO | De eerste indruk',
      excerpt: 'Waarom het telefonisch onthaal voor een onderwijsinstelling het belangrijkste visitekaartje is.',
      theme: 'Stories',
      createdAt: new Date().toISOString(),
      icon: GraduationCap
    }
    ...
    <PageWrapperInstrument className="pt-32 pb-40 px-6 md:px-12 bg-va-off-white min-h-screen">
      <ContainerInstrument className="max-w-7xl mx-auto">
        ...
        <SectionInstrument className="mb-16">
          <ContainerInstrument className="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 rounded-full text-primary text-[15px] font-light tracking-widest border border-primary/10 mb-8 uppercase">
              ...
              <BentoCard key={article.id} span={i === 0 ? "lg" : "sm"} className="bg-white shadow-sm hover:shadow-aura transition-all group overflow-hidden flex flex-col !rounded-[20px]">
                <Link href={`/article/${article.slug}`} className="flex-1 flex flex-col p-8">
                  <ContainerInstrument className="flex items-center gap-4 mb-6">
                    <div className="px-3 py-1 bg-va-off-white rounded-full text-[15px] font-light tracking-widest text-va-black/40 border border-black/5 flex items-center gap-2 uppercase">
                      <Icon size={10} className="text-primary" strokeWidth={1.5} />
                      {theme}
                    </div>
                    <TextInstrument className="flex items-center gap-2 text-[15px] font-light text-va-black/30 tracking-widest uppercase">
                  ...
                  <div className="flex items-center gap-2 text-[15px] font-light tracking-widest text-primary group-hover:gap-4 transition-all uppercase">

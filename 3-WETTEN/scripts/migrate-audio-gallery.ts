...
async function main() {
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    // 1. Extract dossier Actor ID from header
    ...
    if (!supabaseId) {
      console.log(`⚠️ No Supabase ID mapping found for Actor ID [${dossierActorId}]. Skipping.`);
      continue;
    }
    ...
    console.log(`\n🎙️ Processing Audio Gallery for Actor [${dossierActorId}] (Supabase ID: ${supabaseId})...`);
    ...
    for (const line of demoLines) {
      try {
        ...
        const storagePath = `audio-gallery/${supabaseId}/${fileName}`;

...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await getActors(params, lang).catch((err) => {
      ...
      return { results: [], count: 0, _fatal: err.message, _stack: err.stack, filters: { genders: [], languages: [], styles: [] }, reviews: [], reviewStats: { averageRating: 4.9, totalCount: 0, distribution: {} } };
    });

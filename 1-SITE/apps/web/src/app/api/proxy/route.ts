 ...
 * direct van de Combell backend (PHP layer) te fetchen.
 */
...
export async function GET(request: NextRequest) {
  ...
  if (!assetPath) {
    ...
    if (!cleanPath.startsWith('/assets/') && !cleanPath.startsWith('/wp-content/')) {
      return new NextResponse('Forbidden asset path: ' + cleanPath, { status: 403 });
    }

    // De backend URL (Combell PHP server)
    const BACKEND_URL = process.env.BACKEND_URL || 'https://www.voices.be'; 
    
    try {
      // 🛡️ FIX: Zorg dat het pad altijd met een slash begint en geen dubbele slashes heeft

 ...
 * 
 * Kolom 1 (25%): De Stem & Demos
 * Kolom 2 (50%): Het Script (Tekstinvoer)
 * Kolom 3 (25%): Gebruik, Prijs & Bestelknop
 */
export default function ConfiguratorPageClient({ 
  ...
  const handleUsageSwitch = (usageId: any) => {
    console.log(`[Configurator] handleUsageSwitch called with: ${usageId}`);
    
    //  CHRIS-PROTOCOL: Sync with Master Control Journey to prevent state reset
    ...
    if (journeyMap[usageId]) {
      console.log(`[Configurator] Updating Master Control journey to ${journeyMap[usageId]}`);
      updateJourney(journeyMap[usageId]);
    } else {
      console.log(`[Configurator] Updating direct usage to ${usageId}`);
      updateUsage(usageId);
    }

    //  KELLY'S SMART DEFAULTS: Reset spots/years when switching to non-commercial
    if (usageId !== 'commercial') {
      updateSpots(1);
      updateYears(1);
    }
    
    // Forceer herberekening na een korte pauze om state sync te garanderen
    setTimeout(() => {
      if (calculatePricing) {
        console.log(`[Configurator] Forcing recalculation for ${usageId}`);
        calculatePricing();
      }
    }, 250); // Nog iets meer tijd voor de state-machine
  };
  ...
  const handleMediaToggle = (mediaId: string) => {
    ...
    // Check if any variant of this media is already selected (e.g. radio_national, radio_regional)
    ...
    if (existingMedia) {
      if (currentMedia.length > 1) {
        ...
        return; // Altijd minimaal 1
      }
    } else {
      newMedia = [...currentMedia, mediaId];
    }
    
    console.log(`[Configurator] Toggling media: ${mediaId} -> ${JSON.stringify(newMedia)}`);
    updateMedia(newMedia);
    
    // Forceer herberekening
    setTimeout(() => {
      if (calculatePricing) {
        console.log(`[Configurator] Forcing recalculation for media change`);
        calculatePricing();
      }
    }, 100);
  };
    ...
    <ContainerInstrument className={cn(
      ...
      <SectionInstrument className={cn(
          ...
          <ContainerInstrument className="text-center mb-12 space-y-8">
            ...
            <div className="space-y-2">
              <HeadingInstrument level={1} className="text-6xl md:text-8xl font-light tracking-tighter text-va-black leading-none">
                <VoiceglotText 
                  translationKey="configurator.title" 
                  defaultText="Script & Prijs" 
                />
              </HeadingInstrument>
        ...
        <div className={cn(
          ...
          {/* KOLOM 1: DE STEM (25%) - Verbergen in minimalMode want AgencyContent regelt dit */}
          {!minimalMode && (
          ...
          {/* KOLOM 2: HET SCRIPT (50% of 12 kolommen in minimalMode) */}
          <div className={cn(
            "space-y-6 pt-0",
            minimalMode ? "lg:col-span-12" : "lg:col-span-6"
          )}>
            {/* Gebruik & Rechten (Verplaatst van kolom 3) */}
            {!hideMediaSelector && (
              <div className="space-y-4 mb-8 relative">
                <div className="flex items-center justify-between px-2">
                  ...
                  <div className="text-[10px] font-bold text-primary/40 tracking-widest uppercase">
                    Selecteer type
                  </div>
                </div>
                <div 
                  ...
                  {usageTypes.map((type) => {
                    ...
                    return (
                      <button
                        key={type.id}
                        type="button"
                        data-testid={`usage-type-${type.id}`}
                        onClick={(e) => {
                          console.log(`[Configurator] Clicked on ${type.id}`);
                          e.preventDefault();
                          e.stopPropagation();
                      ...
                      >
                        ...
                        <div className="pointer-events-none">
                          <div className={cn(
                            "text-[12px] font-bold tracking-tight leading-tight transition-colors",
                            isActive ? "text-primary" : "text-va-black"
                          )}>{type.label}</div>
                          <div className="text-[9px] text-va-black/30 uppercase tracking-widest font-black mt-0.5">
                            {type.description}
                          </div>
                        </div>
                      </button>
                ...
                {/* Paid Media Options (Kelly's Standard) */}
                {state.usage === 'commercial' && (
                  <div className="pt-4 space-y-6 animate-in fade-in slide-in-from-top-2 duration-500">
                    {/* Country Selection */}
                    <div className="space-y-3">
                      <div className="flex items-center justify-between px-2">
                        <LabelInstrument className="text-[10px] font-bold tracking-[0.15em] text-va-black/30 uppercase">
                          Land van uitzending
                        </LabelInstrument>
                      </div>
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                ...
                                isSelected 
                                  ? "bg-va-black text-white border-va-black shadow-md" 
                                  : "bg-white border-black/[0.03] text-va-black/40 hover:border-black/10"
                              )}
                            >
                              {c.label}
                            </button>
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                isSelected 
                                  ...
                                  : "bg-white border-black/[0.03] text-va-black/40 hover:border-black/10 hover:bg-va-off-white/50"
                              )}
                            >
                    ...
                    {/* KELLY'S GRANULAR MEDIA CONTROLS */}
                        ...
                        {state.media?.map((mediaId) => {
                          ...
                          return (
                            <motion.div 
                              ...
                              className="bg-white rounded-[20px] p-5 shadow-aura border border-black/[0.03] relative overflow-hidden group"
                            >
                                    ...
                                    <div className="flex gap-2">
                                      {regions.map(r => (
                                        <button
                                          key={r.id}
                                          onClick={() => {
                                            // Update media ID to reflect region (e.g. radio_national, radio_regional, radio_local)
                                            const baseId = mediaId.split('_')[0];
                                          ...
                                          className={cn(
                                            ...
                                            mediaId.includes(r.id.toLowerCase())
                                              ? "bg-primary/10 border-primary/20 text-primary"
                                              : "bg-va-off-white/50 border-black/[0.03] text-va-black/40 hover:border-black/10"
                                          )}
                                        >
                                          {r.label}
                                        </button>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                  ...
                                  <div className="space-y-3">
                                    ...
                                    <div className="flex items-center gap-4">
                                      <button 
                                        onClick={() => {
                                          const next = Math.max(1, currentSpots - 1);
                                          updateSpotsDetail({ ...state.spotsDetail, [mediaId]: next });
                                          setTimeout(() => calculatePricing?.(), 50);
                                        }}
                                        className="w-8 h-8 rounded-xl bg-va-off-white border border-black/5 flex items-center justify-center hover:bg-va-black hover:text-white transition-all active:scale-90"
                                      >
                                      ...
                                      <div className="flex-1 relative h-1.5 bg-va-black/5 rounded-full overflow-hidden">
                                        <motion.div 
                                          ...
                                          initial={false}
                                          animate={{ width: `${(currentSpots / 10) * 100}%` }}
                                        />
                                        <input 
                                          type="range" min="1" max="10" step="1"
                                          value={currentSpots}
                                          onChange={(e) => {
                                            updateSpotsDetail({ ...state.spotsDetail, [mediaId]: parseInt(e.target.value) });
                                            setTimeout(() => calculatePricing?.(), 50);
                                          }}
                                      ...
                                      <button 
                                        onClick={() => {
                                          const next = Math.min(10, currentSpots + 1);
                                          updateSpotsDetail({ ...state.spotsDetail, [mediaId]: next });
                                          setTimeout(() => calculatePricing?.(), 50);
                                        }}
                                        className="w-8 h-8 rounded-xl bg-va-off-white border border-black/5 flex items-center justify-center hover:bg-va-black hover:text-white transition-all active:scale-90"
                                  ...
                                  <div className="space-y-3">
                                    <div className="flex justify-between items-center">
                                      ...
                                      <span className="text-[14px] font-black text-primary">
                                        {isPodcast ? `${currentYears * 4} units` : `${currentYears} jaar`}
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-4">
                                      <button 
                                        onClick={() => {
                                          const next = Math.max(isPodcast ? 0.25 : 1, currentYears - (isPodcast ? 0.25 : 1));
                                          updateYearsDetail({ ...state.yearsDetail, [mediaId]: next });
                                          setTimeout(() => calculatePricing?.(), 50);
                                        }}
                                        className="w-8 h-8 rounded-xl bg-va-off-white border border-black/5 flex items-center justify-center hover:bg-va-black hover:text-white transition-all active:scale-90"
                                      >
                                      ...
                                      <div className="flex-1 relative h-1.5 bg-va-black/5 rounded-full overflow-hidden">
                                        <motion.div 
                                          className="absolute inset-y-0 left-0 bg-primary"
                                          initial={false}
                                          animate={{ width: `${(currentYears / 5) * 100}%` }}
                                        />
                                        <input 
                                          type="range" 
                                          min={isPodcast ? 0.25 : 1} 
                                          max="5" 
                                          step={isPodcast ? 0.25 : 1}
                                          value={currentYears}
                                          onChange={(e) => {
                                            updateYearsDetail({ ...state.yearsDetail, [mediaId]: parseFloat(e.target.value) });
                                            setTimeout(() => calculatePricing?.(), 50);
                                          }}
                                          className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                                        />
                                      ...
                                      <button 
                                        onClick={() => {
                                          const next = Math.min(5, currentYears + (isPodcast ? 0.25 : 1));
                                          updateYearsDetail({ ...state.yearsDetail, [mediaId]: next });
                                          setTimeout(() => calculatePricing?.(), 50);
                                        }}
                                        className="w-8 h-8 rounded-xl bg-va-off-white border border-black/5 flex items-center justify-center hover:bg-va-black hover:text-white transition-all active:scale-90"
            ...
            <ContainerInstrument className="bg-white rounded-[20px] shadow-aura border border-black/[0.03] overflow-hidden">
              <textarea
                value={localBriefing}
            ...
            {/* KELLY'S UPSELLS (Live Session & Music) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
              <button
                onClick={() => updateLiveSession(!state.liveSession)}
                className={cn(
                  "flex items-center justify-between p-5 rounded-[20px] border transition-all text-left group",
                  state.liveSession ? "bg-primary/5 border-primary/20" : "bg-white border-black/[0.03] hover:border-black/10"
                )}
              >
                <div className="flex items-center gap-4">
                  <div className={cn(
                    "w-10 h-10 rounded-full flex items-center justify-center transition-colors",
                    state.liveSession ? "bg-primary text-white" : "bg-va-off-white text-va-black/20 group-hover:text-primary"
                  )}>
                ...
                <div className={cn("text-[13px] font-medium", state.liveSession ? "text-primary" : "text-va-black/40")}>
                  + €99
                </div>
              </button>
              ...
              <button
                ...
                className={cn(
                  "flex items-center justify-between p-5 rounded-[20px] border transition-all text-left group",
                  state.music.asBackground ? "bg-primary/5 border-primary/20" : "bg-white border-black/[0.03] hover:border-black/10"
                )}
              >
                <div className="flex items-center gap-4">
                  <div className={cn(
                    "w-10 h-10 rounded-full flex items-center justify-center transition-colors",
                    state.music.asBackground ? "bg-primary text-white" : "bg-va-off-white text-va-black/20 group-hover:text-primary"
                  )}>
                ...
                <div className={cn("text-[13px] font-medium", state.music.asBackground ? "text-primary" : "text-va-black/40")}>
                  + €59
                </div>
              </button>
            </div>

            {/* PRIJS & CTA - Nu onder het tekstvak in minimalMode */}
            {minimalMode && (
              <div className="mt-8">
                        ...
                        {state.pricing.wordSurcharge > 0 && (
                          <div className="flex justify-between text-[13px]">
                            <span className="text-va-black/40 font-light">
                              {state.usage === 'telefonie' ? 'Extra prompts' : 'Extra woorden'}
                            </span>
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-8 justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="text-5xl font-light tracking-tighter text-va-black">
                          {PricingEngine.format(state.pricing.total)}
                        </div>
                      </div>
                      ...
                      <div className="flex-1 w-full space-y-3">
                        ...
                        <button 
                          ...
                          disabled={!state.selectedActor || wordCount === 0 || addedToCart}
                        ...
                        >
                          {addedToCart ? (
                            <><CheckCircle2 size={12} className="text-green-500" /> Toegevoegd</>
                          ) : (
                            <><ShoppingBag size={12} /> Bewaar in mandje</>
                          )}
                        </button>
                      </div>
                    </div>
          ...
          {/* KOLOM 3: PRIJS & RECHTEN (25%) - Verbergen in minimalMode */}
          {!minimalMode && (
              ...
              <ContainerInstrument className="bg-white rounded-[20px] p-8 text-va-black shadow-aura border border-black/[0.03] relative overflow-hidden">
                <div className="space-y-6 relative z-10">
                    ...
                    <div className="space-y-2">
                      ...
                      {state.usage === 'commercial' && state.pricing.mediaSurcharge > 0 && (
                        <div className="flex justify-between text-[13px]">
                          <span className="text-va-black/40 font-light">
                            Buyout ({state.media?.length} {state.media?.length === 1 ? 'kanaal' : 'kanalen'})
                            {state.spots > 1 && ` • ${state.spots} spots`}
                            {state.years > 1 && ` • ${state.years}j`}
                          </span>
                          <span className="font-medium">+{PricingEngine.format(state.pricing.mediaSurcharge)}</span>
                        </div>
                      )}
                  ...
                  <div className="space-y-1 pt-4 border-t border-black/[0.03]">
                    ...
                    {state.pricing.legalDisclaimer && (
                      <div className="text-[10px] text-va-black/40 font-light italic mt-2 leading-tight">
                        {state.pricing.legalDisclaimer}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="pt-4 space-y-3">
                    ...
                    <button 
                      onClick={handleAddToCart}
                      disabled={!state.selectedActor || wordCount === 0 || addedToCart}
                      className="w-full py-3 text-[11px] font-bold tracking-[0.2em] text-va-black/20 hover:text-primary transition-all flex items-center justify-center gap-2 uppercase border border-black/[0.03] rounded-[10px] hover:bg-black/[0.02]"
                    >
                      {addedToCart ? (
                        <><CheckCircle2 size={12} className="text-green-500" /> Toegevoegd</>
                      ) : (

...
import { ArrowLeft, Play, Star, Heart, Share2, Mic, ShieldCheck, Instagram, Youtube, Music } from "lucide-react";
      ...
      <div className={`${isYoussef ? 'max-w-6xl mx-auto px-6 py-20' : ''}`}>
        ...
        <SectionInstrument className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-32 items-start">
          ...
          <div className="lg:col-span-7 pt-8">
            <div className="flex items-center gap-3 mb-6">
              <span className={`px-3 py-1 rounded-full text-[10px] font-light tracking-widest border ${isYoussef ? 'bg-primary/20 text-primary border-primary/20' : 'bg-primary/10 text-primary border border-primary/10'}`}>
            ...
            {isYoussef && (
              <div className="bg-white/5 p-8 rounded-[20px] border border-white/5 mb-12 max-w-xl backdrop-blur-sm">
                <div className="flex justify-between items-end mb-4">
                  <div>
                    <HeadingInstrument level={3} className="text-2xl font-black tracking-tight mb-1 uppercase text-white">
                ...
                >
                  ...
                  <span className="font-black uppercase tracking-widest text-xs">Support Youssef</span>
                </ButtonInstrument>
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-4">
              <ButtonInstrument className={`va-btn-pro !px-10 !py-6 text-base !rounded-[10px] flex items-center gap-2 group ${isYoussef ? '!bg-white/5 !text-white border border-white/10 hover:!bg-white/10' : '!bg-white !text-va-black border border-black/5'}`}>
                ...
                <span className={isYoussef ? 'font-black uppercase tracking-widest text-xs' : ''}>
        ...
        {isYoussef && (
          <SectionInstrument className="mb-32">
            <HeadingInstrument level={2} className="text-4xl font-black tracking-tighter mb-12 text-center uppercase text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="relative aspect-video rounded-[20px] overflow-hidden shadow-aura group cursor-pointer bg-va-black border border-white/5">
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <TextInstrument className="text-white/40 text-[10px] font-black tracking-widest uppercase">The Voice France</TextInstrument>
                  <HeadingInstrument level={4} className="text-white text-xl font-black uppercase tracking-tight">Fix You (Coldplay Cover)</HeadingInstrument>
                </div>
              </div>
              <div className="relative aspect-video rounded-[20px] overflow-hidden shadow-aura group cursor-pointer bg-va-black border border-white/5">
                <Image 
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <TextInstrument className="text-white/40 text-[10px] font-black tracking-widest uppercase">Brussels Street Session</TextInstrument>
                  <HeadingInstrument level={4} className="text-white text-xl font-black uppercase tracking-tight">My Funny Valentine</HeadingInstrument>
                </div>
              </div>
            </div>
          </SectionInstrument>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <TextInstrument className={`text-[10px] tracking-widest uppercase ${isYoussef ? 'font-black text-white/20' : 'font-light text-va-black/20'}`}>
                        {demo.category}
                      </TextInstrument>
                    </div>
                  </div>

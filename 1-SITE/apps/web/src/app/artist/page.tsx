        ...
        >
          <ContainerInstrument className="flex justify-between items-start mb-12 relative z-10">
            <ContainerInstrument className="bg-white/10 text-white/60 text-[15px] font-black px-3 py-1 rounded-full tracking-widest border border-white/5">
              {artist.status === 'live' ? <VoiceglotText strokeWidth={1.5} translationKey="artist.status.available" defaultText="AVAILABLE" / /> : <VoiceglotText strokeWidth={1.5} translationKey="artist.status.recording" defaultText="RECORDING" / />}
            </ContainerInstrument>
            <TextInstrument className="text-[15px] font-black text-orange-500 tracking-widest"><VoiceglotText strokeWidth={1.5} translationKey="artist.badge.sonic_talent" defaultText="Voices" / /></TextInstrument>
          </ContainerInstrument>

          <HeadingInstrument level={3} className="text-4xl font-black tracking-tighter leading-[0.8] mb-4 group-hover:text-orange-400 transition-colors relative z-10 "><VoiceglotText strokeWidth={1.5} 
              translationKey={`artist.${artist.id}.name`} 
              defaultText={`${artist.firstName} ${artist.lastName || ''}`} 
              noTranslate={true}
            / /><TextInstrument className="text-white/40 text-[15px] font-black tracking-[0.2em] mb-12 relative z-10"><VoiceglotText strokeWidth={1.5} translationKey={`artist.${artist.id}.tag`} defaultText={artist.aiTags?.split(',')[0] || 'Musical Artist'} / /></TextInstrument></HeadingInstrument>
          ...
          <ContainerInstrument className="mt-auto flex justify-between items-center relative z-10">
            ...
            ><VoiceglotText strokeWidth={1.5} translationKey="artist.view_portfolio" defaultText="VIEW PORTFOLIO" / /></ButtonInstrument>
    ...
    <PageWrapperInstrument className="min-h-screen pt-32 pb-32 bg-va-black text-white relative overflow-hidden">
      ...
      <SectionInstrument className="px-8 mb-24 max-w-7xl mx-auto">
        <ContainerInstrument className="max-w-5xl">
          <ContainerInstrument className="inline-block bg-white/5 text-white/40 text-[15px] font-black px-6 py-2 rounded-full mb-10 tracking-[0.3em] border border-white/10"><VoiceglotText strokeWidth={1.5} translationKey="artist.hero.badge" defaultText="Voices Artists" / /></ContainerInstrument>
          <HeadingInstrument level={1} className="text-[10vw] md:text-[7vw] font-black tracking-tighter leading-[0.85] mb-10 "><VoiceglotText strokeWidth={1.5} translationKey="artist.hero.title_part1" defaultText="YOUR SOUND, " / /><TextInstrument as="span" className="text-orange-500 font-light"><VoiceglotText strokeWidth={1.5} translationKey="artist.hero.title_highlight" defaultText="UNFILTERED." / /></TextInstrument></HeadingInstrument>
          <TextInstrument className="text-2xl md:text-4xl text-white/30 font-light leading-tight tracking-tight max-w-3xl italic"><VoiceglotText strokeWidth={1.5} 

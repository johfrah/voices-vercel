...
export default async function LaunchpadPage() {
  ...
  let actors = [];
  try {
    const searchResults = await getActors({ status: 'live' }, lang);
    actors = searchResults.results || [];
  } catch (error) {
    console.error('[Launchpad] Failed to fetch actors:', error);
  }
    ...
    <PageWrapperInstrument className="relative min-h-screen pb-20 overflow-hidden">
      ...
      <SectionInstrument className="max-w-7xl mx-auto px-6 relative z-10 !pt-32">
        <Suspense fallback={<div className="w-full h-[600px] bg-va-black/5 rounded-[32px] animate-pulse" />}>
          <StudioLaunchpad strokeWidth={1.5} initialActors={actors} />
        </Suspense>
      </SectionInstrument>

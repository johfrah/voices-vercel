      ...
      <SectionInstrument className="va-section-grid">
        <ContainerInstrument className="va-container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {isAdmin && (
            <BentoCard 
              span="sm" 
              className="bg-white shadow-aura p-12 rounded-[40px] border border-black/[0.03] flex flex-col justify-between transition-all hover:scale-[1.02]"
            ...
            >
              <ContainerInstrument className="space-y-4">
                ...
                <HeadingInstrument level={3} className="text-3xl font-black uppercase tracking-tight mb-4">
              ...
              <ContainerInstrument className="flex items-center gap-2 font-black uppercase tracking-widest text-[10px] mt-8 transition-all text-blue-500">
          ...
          <BentoCard 
            span="sm"
            className="bg-va-dark-soft text-white p-12 rounded-[40px] border border-black/[0.03] flex flex-col justify-between transition-all hover:scale-[1.02]"
          ...
          >
            <ContainerInstrument className="space-y-4">
              ...
              <HeadingInstrument level={3} className="text-3xl font-black uppercase tracking-tight mb-4 text-white">
                <VoiceglotText translationKey="account.card.orders.title" defaultText="Bestellingen" />
              </HeadingInstrument>
              <TextInstrument className="text-white/40 text-xs font-medium leading-relaxed max-w-xs">
                <VoiceglotText translationKey="account.card.orders.desc" defaultText="Bekijk de status van je projecten." />
              </TextInstrument>
            </ContainerInstrument>
            <ContainerInstrument className="flex items-center gap-2 font-black uppercase tracking-widest text-[10px] mt-8 transition-all text-white">
          ...
          >
            ...
            <ContainerInstrument className="flex items-center gap-2 font-black uppercase tracking-widest text-[10px] mt-8 transition-all text-primary">
      ...
      {customerDNA && (
        <SectionInstrument className="va-section-grid pt-0">
          <ContainerInstrument className="va-container grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {/* Lead Vibe & Activity */}
            <BentoCard span="sm" className="bg-white border border-black/5 p-10 flex flex-col justify-between">
              <ContainerInstrument>
                <div className="flex justify-between items-start mb-8">
                  <div className="w-12 h-12 bg-primary/10 text-primary rounded-2xl flex items-center justify-center">
                    <Activity size={24} />
                  </div>
                  <ContainerInstrument className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${
              ...
              <ContainerInstrument className="pt-8 border-t border-black/5 mt-8">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black uppercase tracking-widest text-va-black/20">
            ...
            {/* AI Insights (The DNA) */}
            <BentoCard span="lg" className="bg-va-black text-white p-12 relative overflow-hidden group">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customerDNA.dna.preferredLanguages.map((lang: string) => (
                        <span key={lang} className="px-2 py-1 bg-white/5 border border-white/10 rounded text-[9px] font-bold uppercase">{lang}</span>
                      )) || <span className="text-[9px] text-white/40 italic"><VoiceglotText translationKey="common.no_data" defaultText="Nog geen data" /></span>}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/20 block">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customerDNA.dna.topJourneys.map((j: string) => (
                        <span key={j} className="px-2 py-1 bg-primary/20 border border-primary/30 rounded text-[9px] font-bold uppercase text-primary">{j}</span>
                      )) || <span className="text-[9px] text-white/40 italic"><VoiceglotText translationKey="common.no_data" defaultText="Nog geen data" /></span>}
                    </div>
                  </div>
                </div>
          ...
          <ContainerInstrument className="flex flex-col md:flex-row gap-12">
            <ContainerInstrument className="flex-1 space-y-6">
              <ContainerInstrument className="flex items-center gap-3 text-primary">
                ...
                <HeadingInstrument level={2} className="text-3xl font-black uppercase tracking-tighter">
                  <VoiceglotText translationKey="account.security.title" defaultText="Beveiliging & Privacy" />
                </HeadingInstrument>
              </ContainerInstrument>
              <TextInstrument className="text-va-black/40 font-medium leading-relaxed max-w-md">
                <VoiceglotText 
              ...
              <ContainerInstrument className="flex gap-4">
                <ContainerInstrument className="px-4 py-2 bg-white rounded-xl border border-black/5 text-[9px] font-black uppercase tracking-widest text-va-black/40">

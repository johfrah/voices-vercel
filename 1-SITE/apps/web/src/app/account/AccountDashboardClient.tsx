        ...
        <ContainerInstrument className="va-container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {isAdmin && (
            ...
            >
              ...
              <ContainerInstrument className="flex items-center gap-2 font-black tracking-widest text-[15px] mt-8 transition-all text-blue-500">
          ...
          >
            <ContainerInstrument className="space-y-4">
              ...
              <HeadingInstrument level={3} className="text-3xl font-black tracking-tight mb-4 text-white">
                <VoiceglotText translationKey="account.card.orders.title" defaultText="Bestellingen" />
              </HeadingInstrument>
              <TextInstrument className="text-white/40 text-[15px] font-medium leading-relaxed max-w-xs">
                <VoiceglotText translationKey="account.card.orders.desc" defaultText="Bekijk de status van je projecten." />
              </TextInstrument>
            </ContainerInstrument>
            <ContainerInstrument className="flex items-center gap-2 font-black tracking-widest text-[15px] mt-8 transition-all text-white">
          ...
          >
            ...
            <ContainerInstrument className="flex items-center gap-2 font-black tracking-widest text-[15px] mt-8 transition-all text-primary">
            ...
            <BentoCard span="sm" className="bg-white border border-black/5 p-10 flex flex-col justify-between">
              ...
              <ContainerInstrument className="pt-8 border-t border-black/5 mt-8">
                <div className="flex justify-between items-center">
                  <span className="text-[15px] font-black tracking-widest text-va-black/20">
                    <VoiceglotText translationKey="common.status" defaultText="Status" />
                  </span>
                  <span className="text-[15px] font-black tracking-widest text-primary">
              ...
              <ContainerInstrument className="relative z-10">
                ...
                <div className="grid grid-cols-2 gap-8 mt-8">
                  <div className="space-y-2">
                    <span className="text-[15px] font-black tracking-widest text-white/20 block">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customerDNA.dna.preferredLanguages.map((lang: string) => (
                        <span key={lang} className="px-2 py-1 bg-white/5 border border-white/10 rounded text-[15px] font-bold ">{lang}</span>
                      )) || <span className="text-[15px] text-white/40 italic"><VoiceglotText translationKey="common.no_data" defaultText="Nog geen data" /></span>}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <span className="text-[15px] font-black tracking-widest text-white/20 block">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {customerDNA.dna.topJourneys.map((j: string) => (
                        <span key={j} className="px-2 py-1 bg-primary/20 border border-primary/30 rounded text-[15px] font-bold text-primary">{j}</span>
                      )) || <span className="text-[15px] text-white/40 italic"><VoiceglotText translationKey="common.no_data" defaultText="Nog geen data" /></span>}
                    </div>
                  </div>
                </div>
          ...
          <ContainerInstrument className="flex flex-col md:flex-row gap-12">
            <ContainerInstrument className="flex-1 space-y-6">
              <ContainerInstrument className="flex items-center gap-3 text-primary">
                ...
                <HeadingInstrument level={2} className="text-3xl font-black tracking-tighter">
              ...
              <ContainerInstrument className="flex gap-4">
                <ContainerInstrument className="px-4 py-2 bg-white rounded-xl border border-black/5 text-[15px] font-black tracking-widest text-va-black/40">

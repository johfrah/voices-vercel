...
export const WorkshopCalendar: React.FC<{ workshops: any[] }> = ({ workshops }) => {
  ...
  return (
    <ContainerInstrument className="bg-white/40 backdrop-blur-md border border-white/20 rounded-[20px] p-6 md:p-8 shadow-aura group/calendar">
      <ContainerInstrument className="flex items-center justify-between mb-6 md:mb-8">
        <ContainerInstrument className="flex items-center gap-3 md:gap-4">
          <ContainerInstrument className="w-10 h-10 md:w-12 md:h-12 rounded-[10px] bg-va-black text-white flex items-center justify-center shadow-lg group-hover/calendar:bg-primary transition-all duration-500">
            <Image  src="/assets/common/branding/icons/INFO.svg" width={24} height={24} alt="" className="brightness-0 invert" />
          </ContainerInstrument>
          <ContainerInstrument>
            <HeadingInstrument level={3} className="text-xl md:text-2xl font-light tracking-tight text-va-black">
        ...
        <ContainerInstrument className="flex gap-2">
          <ButtonInstrument 
            ...
            className="w-10 h-10 rounded-[10px] bg-white border border-va-black/5 flex items-center justify-center text-va-black/40 hover:text-primary transition-all shadow-sm active:scale-95 bg-transparent p-0"
          >
      ...
      {/* Mini Calendar Grid */}
      <ContainerInstrument className="grid grid-cols-7 gap-1 md:gap-2 mb-6 md:mb-8">
        {days.map(day => (
        ...
        {[...Array(28)].map((_, i) => {
          ...
          return (
            <ButtonInstrument 
              ...
              className={cn(
                "aspect-square rounded-[10px] flex items-center justify-center text-[15px] font-light transition-all cursor-pointer active:scale-90 p-0",
                hasWorkshop 
      ...
      <ContainerInstrument className="space-y-3 md:space-y-4">
        <HeadingInstrument level={4} className="text-[15px] font-light tracking-widest text-va-black/40 mb-3 md:mb-4 ">
        ...
        {workshops.slice(0, 2).map((workshop, i) => (
          <ButtonInstrument 
            ...
            className="w-full p-3 md:p-4 rounded-[20px] bg-white border border-va-black/5 flex items-center justify-between group hover:border-primary/20 hover:shadow-aura transition-all cursor-pointer active:scale-[0.98] bg-transparent"
          >
            <ContainerInstrument className="flex items-center gap-3 md:gap-4">
              ...
              <ContainerInstrument className="text-left">
                ...
                <ContainerInstrument className="flex items-center gap-2 md:gap-3 mt-1">
                  <ContainerInstrument className="flex items-center gap-1 text-[15px] font-light text-va-black/30 tracking-widest ">

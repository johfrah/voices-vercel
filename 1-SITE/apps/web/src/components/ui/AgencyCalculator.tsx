    ...
    switch (calcUsage) {
      ...
      case 'paid':
        return {
          ...
          steps: [
            { num: 1, title: t('calculator.paid.step1.title', lang === 'fr' ? 'L\'Enregistrement' : lang === 'en' ? 'The Recording' : "De Opname"), desc: t('calculator.paid.step1.desc', lang === 'fr' ? `Pour toutes les voix op Voices, le même tarif fixe de €${currentBsf} s'applique pour le temps de studio. C'est la base de votre session.` : lang === 'en' ? `For all voices on Voices, the same fixed rate of €${currentBsf} applies for studio time. This is the basis for your session.` : `Voor alle stemmen op Voices geldt hetzelfde vaste tarief van €${currentBsf} voor de studiotijd. Dit is de basis voor je sessie.`, { price: currentBsf }) },
            ...
            { num: 3, title: t('calculator.paid.step3.title', lang === 'fr' ? 'Direction Live (Optionnel)' : lang === 'en' ? 'Live Direction (Optional)' : "Live Regie (Optioneel)"), desc: t('calculator.paid.step3.desc', lang === 'fr' ? `Dirigez la voix en direct pendant la session. Le tarif voor cela est déterminé par le comédien voix choisi.` : lang === 'en' ? `Direct the voice live during the session. The rate for this is determined by the chosen voice actor.` : `Regisseer de stem live tijdens de sessie. Het tarief hiervoor wordt bepaald door de gekozen stemacteur.`) }
          ]
        };
      default:
        return {
          ...
          <div className="pt-8 border-t border-black/[0.03] space-y-6">
            ...
            <div className="flex items-center gap-3 px-5 py-3 bg-va-off-white rounded-full border border-black/5 w-fit shadow-sm">
              ...
              <TextInstrument className="text-[11px] font-bold text-va-black/40 tracking-[0.2em] uppercase">
                <VoiceglotText translationKey="common.guaranteed_quality" defaultText={`Gegarandeerde ${MarketManager.getCurrentMarket().name} Kwaliteit`} />
              </TextInstrument>
            </div>
          </div>
        </div>

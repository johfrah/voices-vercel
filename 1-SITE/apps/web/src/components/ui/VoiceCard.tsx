...
export const VoiceCard: React.FC<VoiceCardProps> = ({ voice, onSelect }) => {
  ...
  return (
    <BentoCard 
      span="md" 
      className={cn(
        "group cursor-pointer bg-white border rounded-[20px] p-6 md:p-8 transition-all duration-700 hover:shadow-aura hover:-translate-y-1 relative overflow-hidden va-interactive",
    ...
    >
      ...
      <ContainerInstrument 
        className={cn(
          "absolute -top-20 -right-20 w-48 md:w-64 h-48 md:h-64 rounded-full blur-[60px] md:blur-[80px] transition-all duration-1000",
      ...
      <ContainerInstrument className="relative z-10 flex flex-col h-full justify-between">
        <ContainerInstrument>
          ...
          <ContainerInstrument className="flex items-start gap-4 md:gap-5 mb-6 md:mb-8">
            <ContainerInstrument className="relative w-20 h-20 md:w-28 md:h-28 rounded-[24px] md:rounded-[32px] overflow-hidden shadow-2xl group-hover:scale-105 transition-transform duration-700 shrink-0">
              {voice.photo_url ? (
                <VoiceglotImage  
                  src={voice.photo_url} 
              ...
              <ButtonInstrument 
                ...
                className={cn(
                  "absolute top-2 right-2 p-1.5 md:p-2 rounded-full backdrop-blur-md flex items-center justify-center transition-all duration-300 z-20 border border-white/20",
                  isSelected 
            ...
            <ContainerInstrument className="flex-1 min-w-0">
              <ContainerInstrument className="flex flex-col gap-0.5 md:gap-1 mb-2 md:mb-3">
                <HeadingInstrument level={3} className="text-xl md:text-3xl font-light tracking-tighter text-va-black truncate">
                  <VoiceglotText  
                ...
                <ContainerInstrument className="flex items-center gap-1.5 md:gap-2">
                  <TextInstrument className="text-[15px] md:text-[15px] font-light text-va-black/30 tracking-[0.2em] ">
          ...
          <ContainerInstrument className="flex flex-wrap gap-1.5 md:gap-2 mb-4 md:mb-6">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <ButtonInstrument
                  ...
                  className={cn(
                    "flex items-center gap-1 md:gap-1.5 px-2 md:px-3 py-1.5 md:py-2 rounded-lg md:rounded-xl border transition-all duration-300",
                    isActive 
                ...
                >
                  <ContainerInstrument className="w-2.5 h-2.5 md:w-3 md:h-3 relative">
                    <VoiceglotImage  
                      src={cat.src} 
                      alt={cat.label} 
                      fill 
          ...
          {sectorDemo ? (
            <ContainerInstrument className="mb-4 md:mb-6 p-4 md:p-5 bg-primary/5 rounded-[20px] md:rounded-3xl border border-primary/10 animate-in fade-in zoom-in-95 duration-700">
              <ContainerInstrument className="flex items-center gap-2 mb-2 text-primary">
                ...
                />
                <TextInstrument className="text-[15px] md:text-[15px] font-light tracking-[0.2em] ">
                  <VoiceglotText  translationKey="common.for_your_sector" defaultText="Voor uw sector" />
                </TextInstrument>
              </ContainerInstrument>
              <TextInstrument className="text-[15px] md:text-[15px] font-light text-va-black/60 italic leading-relaxed">
                &quot;{sectorDemo}&quot;
              </TextInstrument>
            </ContainerInstrument>
          ) : (
            <ContainerInstrument className="mb-6 md:mb-8">
              {(voice as any).bio ? (
                <TextInstrument className="text-[15px] md:text-[15px] text-va-black/50 font-light leading-relaxed italic mb-3 md:mb-4 line-clamp-2">
                  &quot;<VoiceglotText  translationKey={`actor.${voice.id}.bio`} defaultText={(voice as any).bio} />&quot;
                </TextInstrument>
              ) : (
                <TextInstrument className="text-[15px] md:text-[15px] text-va-black/50 font-light leading-relaxed italic mb-3 md:mb-4">
              ...
              {voice.ai_tags && (
                <ContainerInstrument className="flex flex-wrap gap-1.5 md:gap-2">
                  {(typeof voice.ai_tags === 'string' ? voice.ai_tags.split(',') : (Array.isArray(voice.ai_tags) ? voice.ai_tags : [])).map((tag: any, i: number) => {
                    ...
                    return (
                      <TextInstrument as="span" key={i} className="inline-flex items-center gap-1 md:gap-1.5 px-2 md:px-3 py-0.5 md:py-1 bg-va-off-white rounded-full text-[15px] md:text-[15px] font-light tracking-widest text-va-black/40 border border-black/5 ">
                        <VoiceglotText  translationKey={`common.tag.${label.toLowerCase()}`} defaultText={label} />
                        {isAi && <TextInstrument as="span" className="text-[15px] md:text-[15px] bg-primary/10 text-primary px-1 rounded-sm font-light">AI</TextInstrument>}
                      </TextInstrument>
                    );
                  })}
                </ContainerInstrument>
        ...
        {/* Footer: Price & Action */}
        <ContainerInstrument className="pt-4 md:pt-6 border-t border-black/5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <ContainerInstrument>
            <ContainerInstrument className="flex items-center gap-1.5 text-[15px] md:text-[15px] font-light text-va-black/30 tracking-widest mb-0.5 md:mb-1 ">
              <VoiceglotImage  
                src="/assets/common/branding/icons/INFO.svg" 
                alt="Delivery" 
                width={10} 
          ...
          <ButtonInstrument 
            ...
            className="w-full sm:w-auto bg-va-dark text-white px-4 md:px-6 py-3 md:py-4 rounded-[10px] text-[15px] md:text-[15px] font-light tracking-widest hover:bg-primary transition-all duration-500 transform hover:scale-105 active:scale-95 shadow-xl shadow-black/5 flex items-center justify-center gap-2 "
          >

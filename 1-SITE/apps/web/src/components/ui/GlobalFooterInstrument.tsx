...
import { VoicesLinkInstrument } from './VoicesLinkInstrument';
import { ThemeToggle } from '@/worlds/6-ademing/components/ThemeToggle';
...
import { JourneyCta } from './JourneyCtaInstrument';
...
import { AgencyFooter } from '@/worlds/1-agency/components/AgencyFooterInstrument';
import { StudioFooter } from '@/worlds/2-studio/components/StudioFooterInstrument';
import { AcademyFooter } from '@/worlds/3-academy/components/AcademyFooterInstrument';
import { PortfolioFooter } from '@/worlds/5-portfolio/components/PortfolioFooterInstrument';
import { ArtistFooter } from '@/worlds/25-artist/components/ArtistFooterInstrument';
        ...
        <ContainerInstrument className="flex flex-col md:flex-row items-center gap-8">
          <TextInstrument className="flex items-center gap-2 text-[15px] font-light tracking-widest text-va-black/20 ">
            ...
            <ContainerInstrument className="flex items-center gap-6">
              <VoicesLinkInstrument href="/agency/voorwaarden" className="text-[11px] font-bold uppercase tracking-[0.2em] text-va-black/20 hover:text-va-black transition-colors">
                <VoiceglotText translationKey="footer.link.terms" defaultText="Voorwaarden" />
              </VoicesLinkInstrument>
              <VoicesLinkInstrument href="/privacy" className="text-[11px] font-bold uppercase tracking-[0.2em] text-va-black/20 hover:text-va-black transition-colors">
            ...
            {!isSpecial && (
              <ContainerInstrument className="flex items-center gap-6 opacity-20 grayscale hover:grayscale-0 hover:opacity-50 transition-all duration-700">
                {checkoutState.paymentMethods.length > 0 ? (
                  checkoutState.paymentMethods.map((m: any) => (
                    m.image?.size2x && (
                      <Image 
                        key={m.id}
                        src={m.image.size2x} 

...
export const ReviewsInstrument: React.FC<{ 
  ...
  // Auto-scroll logic (Infinite Marquee Vibe)
  useEffect(() => {
    if (isLoading || isEditMode || isHovered) return;
  ...
  }, [isLoading, isEditMode, isHovered]);
  ...
  const filteredReviews = useMemo(() => {
    let base = localReviews.filter(r => {
      ...
      // In edit mode laten we ook hidden reviews zien (transparant)
  ...
  return (
    <ContainerInstrument plain className="w-full relative group/reviews">
      ...
      <ContainerInstrument 
        ...
        className="flex overflow-x-auto pb-12 -mx-6 px-6 snap-x snap-mandatory no-scrollbar gap-8"
      >
        {isLoading ? (

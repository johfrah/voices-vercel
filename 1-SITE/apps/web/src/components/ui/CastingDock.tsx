        ...
        >
          <ContainerInstrument className="bg-va-black text-white rounded-[25px] p-3 md:p-4 shadow-aura-lg flex items-center justify-between gap-3 border border-white/10 backdrop-blur-md bg-va-black/90">
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1 flex-1">
              <div className="flex -space-x-3 mr-1 shrink-0">
                {selectedActors.map((actor) => (
                  <div 
                    key={actor.id}
                    className="relative group"
                  >
                    <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-full border-2 border-va-black bg-va-off-white overflow-hidden">
                      {actor.photoUrl ? (

...
export const VoicesDropdown: React.FC<VoicesDropdownProps> = ({
  ...
  const updateStepper = (itemValue: string, delta: number) => {
    ...
    // 🛡️ CHRIS-PROTOCOL: Exclusivity Logic (Radio & TV)
    if (nextValue > 0) {
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="py-2 max-h-[400px] overflow-y-auto no-scrollbar">
              {!required && !multiSelect && !stepperMode && !searchQuery && (
                <button
              ...
              {filteredOptions.map((opt, idx) => {
                ...
                if (stepperMode) {
                  ...
                  return (
                    <div key={idx} className="px-6 py-4 flex items-center justify-between group hover:bg-va-off-white transition-colors border-b border-black/[0.03] last:border-0">
                      
                      <div className="flex items-center gap-4 min-w-0">
                        {IconComponent && (
                          <div className={cn("shrink-0", count > 0 ? "text-primary" : "text-va-black/30")}>
                            {typeof IconComponent === 'string' ? (
                              <VoiceglotImage src={IconComponent} width={20} height={20} alt="" />
                            ) : (
                ...
                const isSelected = Array.isArray(value) ? value.includes(item.value) : (typeof value === 'string' ? value === item.value : false);

...
import { MediaChannel, SlimmeKassa, Region } from '@/lib/pricing-engine';
import { cn } from '@/lib/utils';
import { Check, ChevronRight, Megaphone, Music, Phone, Video, Radio, Tv, Globe, Mic2, Plus, Minus, Zap } from 'lucide-react';
...
interface PricingCalculatorProps {
  mode?: 'human' | 'ai' | 'widget';
  actor?: any;
}

export const PricingCalculator: React.FC<PricingCalculatorProps> = ({ mode = 'human', actor }) => {
  const { state, updateUsage, updateBriefing, updateMusic, setStep, selectActor, updateSecondaryLanguages } = useCheckout();
  ...
  const [secondaryLanguages, setSecondaryLanguagesState] = useState<string[]>([]);
  ...
  const pricingConfig = state.pricingConfig || SlimmeKassa.getDefaultConfig();

  // Sync secondary languages with context
  useEffect(() => {
    if (state.usage === 'telefonie') {
      updateSecondaryLanguages(secondaryLanguages);
    }
  }, [secondaryLanguages, state.usage, updateSecondaryLanguages]);

  //  Fetch & Filter Voices based on criteria
  useEffect(() => {
    const fetchVoices = async () => {
      if (mode === 'widget' && isMinimized) return;
      try {
        ...
        // Filter op taal en journey beschikbaarheid (VoiceFilterEngine Mandate 2026)
        const selectedActor = allActors.find((a: any) => a.native_lang_label === language || a.native_lang === language);
        const relevant = VoiceFilterEngine.filter(allActors, {
          journey: state.usage === 'paid' ? 'commercial' : (state.usage === 'telefonie' ? 'telephony' : 'video'),
          language: language,
          languageId: selectedActor?.native_lang_id || null,
        ...
        setFilteredActors([]); // Fallback naar lege lijst
      }
    };
    fetchVoices();
  }, [language, state.usage, mode, isMinimized]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const actorRates = actor || (referenceActor ? {
        first_name: referenceActor.first_name || 'Stem',
        ai_enabled: referenceActor.ai_enabled || false,
        price_unpaid: referenceActor.price_unpaid,
        price_ivr: referenceActor.price_ivr,
        rates: referenceActor.rates_raw || referenceActor.rates || {}
      } : {
      ...
      const wordCount = Math.max(0, Number(words) || 0);
      const promptCount = state.usage === 'telefonie' ? Math.ceil(wordCount / 8) : 0;
      ...
      const result = SlimmeKassa.calculate({
        ...
        mediaTypes: state.usage === 'paid' ? (media as any) : [],
        spots: media.reduce((acc, m) => ({ ...acc, [m]: spots }), {}),
        years: media.reduce((acc, m) => ({ ...acc, [m]: years }), {}),
        liveSession: liveSession,
        music: { asBackground: state.music.asBackground || state.music.asHoldMusic, asHoldMusic: false },
        actorRates: actorRates
      }, pricingConfig);
      ...
      // 🛡️ USER-MANDATE: We don't block calculation if it's unavailable, 
      // instead we let the Pricing Engine handle it.
      // if (status === 'unavailable' && state.usage === 'paid') {
      //   setPricing({ price: 0, formatted: t('common.unavailable', 'Niet beschikbaar') });
      //   return;
      // }

      setPricing({ 
        price: result.total || 0, 
        formatted: SlimmeKassa.format(result.total) || '€ 0,00' 
      });
    } catch (err) {
      console.error('Pricing calculation failed:', err);
      setPricing({ price: 0, formatted: '€ 0,00' });
    }
  }, [state.usage, words, country, media, mediaRegion, spots, years, mode, actor, filteredActors, state.music.asBackground, state.music.asHoldMusic, t, isMinimized, liveSession]);
  ...
  const handleBookNow = () => {
    ...
    // 🛡️ USER-MANDATE: We don't block booking if it's unavailable, 
    // instead we let the Pricing Engine handle the price display.
    /*
    if (status === 'unavailable' && state.usage === 'paid') {
      const event = new CustomEvent('voicy:suggestion', {
        detail: {
          title: t('common.unavailable', 'Niet beschikbaar'),
          content: t('pricing.suggestion.unavailable', `Het lijkt erop dat ${actor?.display_name || 'deze stem'} geen tarieven heeft ingesteld voor deze media-combinatie en daarom niet direct geboekt kan worden. Wil je dat ik een alternatieve stem voor je zoek of Johfrah vraag om een uitzondering?`),
          tab: 'mail'
        }
      });
    ...
    */
    ...
    if (pricing.price === 0) {
      const event = new CustomEvent('voicy:suggestion', {
        detail: {
          title: t('pricing.error.zero_price_title', 'Oeps, een tarieffout'),
          content: t('pricing.error.zero_price_desc', 'Het lijkt erop dat er een onregelmatigheid is in het tarief voor deze opdracht (0 euro). Om fouten te voorkomen heb ik de bestelling even gepauzeerd. Zal ik Johfrah een seintje geven om dit direct voor je recht te zetten?'),
          tab: 'mail'
        }
      });
      if (typeof window !== 'undefined') {
  ...
  const isWidgetMinimized = mode === 'widget' && isMinimized;

  return (
    <>
      {isWidgetMinimized ? (
        <motion.div 
          layoutId="pricing-widget"
          onClick={() => setIsMinimized(false)}
          className="fixed bottom-8 right-8 bg-va-black text-white p-6 rounded-[32px] shadow-aura-lg cursor-pointer flex items-center gap-4 group z-50 border border-white/10"
        >
          ...
          <div>
            <TextInstrument className="text-[15px] font-light tracking-tight block">Instant Quote</TextInstrument>
            <TextInstrument className="text-[11px] text-white/40 uppercase tracking-[0.2em]">Bereken je prijs</TextInstrument>
          </div>
        </motion.div>
      ) : (
        <motion.div 
          ...
          className={cn(
            "overflow-hidden !p-0",
            mode === 'widget' ? "fixed bottom-8 right-8 w-[90vw] max-w-4xl max-h-[80vh] overflow-y-auto z-50 bg-white rounded-[40px] shadow-2xl border border-black/5" : "relative"
          )}
        >
          {mode === 'widget' && (
            <button 
              onClick={() => setIsMinimized(true)}
              className="absolute top-6 right-6 w-10 h-10 bg-va-off-white rounded-full flex items-center justify-center hover:bg-black/5 transition-colors z-20"
            >
              <Minus size={20} />
            </button>
          )}
          <ContainerInstrument className="flex flex-col lg:flex-row">
            ...
            <ContainerInstrument className="flex-1 p-8 lg:p-12 space-y-10">
              <ContainerInstrument>
                <HeadingInstrument level={3} className="text-[15px] font-light text-va-black/30 mb-6 uppercase tracking-[0.2em]">
                    ...
                    <div className="space-y-4">
                      ...
                      <AnimatePresence>
                        {state.usage === 'telefonie' && (
                          ...
                          >
                            <div className="bg-va-off-white/50 p-6 rounded-[32px] border border-black/5 space-y-4 mt-2">
                              <div className="flex items-center justify-between">
                                <TextInstrument className="text-[11px] text-va-black/30 font-bold uppercase tracking-[0.2em]">
                                  <VoiceglotText translationKey="pricing.multilingual_options" defaultText="Meertalige opties" />
                                </TextInstrument>
                                <div className="text-[11px] text-primary font-medium uppercase tracking-[0.1em]">
                                  {secondaryLanguages.length > 0 ? `${secondaryLanguages.length} extra ${secondaryLanguages.length === 1 ? 'taal' : 'talen'}` : 'Alleen hoofdrtaal'}
                                </div>
                              </div>
                              
                              <div className="flex flex-wrap gap-2">
                                {[
                                  { id: 'en', label: 'Engels', icon: '🇬🇧' },
                                  { id: 'fr', label: 'Frans', icon: '🇫🇷' },
                                  { id: 'de', label: 'Duits', icon: '🇩🇪' },
                                  { id: 'es', label: 'Spaans', icon: '🇪🇸' },
                                  { id: 'it', label: 'Italiaans', icon: '🇮🇹' }
                                ].filter(l => !language.startsWith(l.id)).map((lang) => {
                                  const isSelected = secondaryLanguages.includes(lang.id);
                                  return (
                                    <button
                                      key={lang.id}
                                      onClick={() => {
                                        if (isSelected) {
                                          setSecondaryLanguagesState(secondaryLanguages.filter(l => l !== lang.id));
                                        } else {
                                          setSecondaryLanguagesState([...secondaryLanguages, lang.id]);
                                        }
                                      }}
                                      className={cn(
                                        "flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-medium transition-all border-2",
                                        isSelected 
                                          ? "bg-white border-primary text-primary shadow-sm" 
                                          : "bg-white/50 border-black/5 text-va-black/40 hover:border-black/10"
                                      )}
                                    >
                                      <span>{lang.icon}</span>
                              ...
                              <TextInstrument className="text-[12px] text-va-black/40 font-light italic leading-relaxed">
                                <VoiceglotText 
                                  translationKey="pricing.multilingual_hint" 
                                  defaultText="Selecteer de extra talen die deze stem moet inspreken voor een meertalige productie." 
                                />
                              </TextInstrument>
                            </div>
                          </motion.div>
              ...
              {state.usage === 'paid' ? (
                <ContainerInstrument className="space-y-10 animate-in fade-in slide-in-from-top-4 duration-700">
                  <ContainerInstrument className="space-y-6">
                    <HeadingInstrument level={3} className="text-[15px] font-light text-va-black/30 uppercase tracking-[0.2em]">
                    ...
                    <ContainerInstrument className="flex flex-col gap-4">
                      <div className="grid grid-cols-1 gap-3">
                        ...
                        <div className="flex flex-col">
                          <div className={cn("text-[14px] font-light tracking-tight", isActive ? "text-va-black" : "text-va-black/40")}>{m.label}</div>
                          <div className="text-[11px] text-va-black/20 font-light uppercase tracking-[0.2em]">{m.sub}</div>
                        </div>
                                </div>
                                <div className={cn(
                                  ...
                                  <div className="bg-va-off-white/50 p-6 rounded-[32px] border border-black/5 space-y-4">
                                    <div className="flex items-center justify-between">
                                      <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                          <TextInstrument className="text-[14px] font-light tracking-tight text-va-black">
                                            {m.label} Commercial
                                          </TextInstrument>
                                          <TextInstrument className="text-[11px] text-va-black/30 font-light uppercase tracking-[0.2em]">
                                            Uitzendgebied
                                          </TextInstrument>
                                        </div>
                                      </div>
                                    ...
                                    <div className="flex items-center gap-3 bg-white p-1 rounded-2xl border border-black/5 shadow-inner">
                                      ...
                                      <span className="flex-1 text-[14px] font-light text-primary text-center uppercase tracking-[0.1em]">
                    ...
                    <ContainerInstrument className="grid grid-cols-2 gap-8">
                      <ContainerInstrument className="space-y-4">
                        <TextInstrument className="text-[15px] font-light text-va-black/30 tracking-[0.2em] uppercase">
                        ...
                        <TextInstrument className="text-[15px] font-light text-va-black/30 tracking-[0.2em] uppercase">
                          <VoiceglotText  translationKey="pricing.duration_years" defaultText="Looptijd (Jaar)" />
                                            ...
                                            <div className="flex items-center gap-3 bg-va-off-white p-1 rounded-lg border border-black/5">
                                              ...
                                              >-</button>
                                              <span className="text-[12px] font-light text-primary min-w-[45px] text-center uppercase tracking-[0.1em]">
                                                {years === 0.25 ? t('common.3_months', '3 mnd') : 
                                                 years === 0.5 ? t('common.6_months', '6 mnd') : 
                                                 `${years} ${years === 1 ? t('common.year', 'jaar') : t('common.years', 'jaar')}`}
                                              </span>
                                              <button 
                                                onClick={() => {
                                                  const current = years;
                ...
                <ContainerInstrument className="pt-6 border-t border-black/5 animate-in fade-in duration-500">
                  <ContainerInstrument className="flex items-center justify-between mb-6">
                    <ContainerInstrument className="flex items-center gap-3">
                      ...
                      <ContainerInstrument>
                        <HeadingInstrument level={3} className="text-[15px] font-light tracking-[0.1em] uppercase">
                    ...
                    <ContainerInstrument className="text-right">
                      <TextInstrument className="text-lg font-extralight text-va-black">{(pricingConfig.musicSurcharge / 100).toFixed(0)}</TextInstrument>
                    </ContainerInstrument>
                  </ContainerInstrument>
              ...
              {state.usage !== 'telefonie' && (() => {
                const referenceActor = actor || (filteredActors && filteredActors.length > 0 ? filteredActors[0] : null);
                const hasLiveRegie = referenceActor && (
                  referenceActor.price_live_regie > 0 || 
                  (referenceActor.rates_raw && Object.values(referenceActor.rates_raw).some((r: any) => r.live_regie > 0))
                );
                  ...
                  <ContainerInstrument className="pt-6 border-t border-black/5 animate-in fade-in duration-500">
                    <ContainerInstrument className="flex items-center justify-between mb-6">
                      <ContainerInstrument className="flex items-center gap-3">
                        <ContainerInstrument className={cn(
                          "w-10 h-10 rounded-full flex items-center justify-center transition-all",
                          liveSession ? "bg-primary/10 text-primary" : "bg-va-black/5 text-va-black/20"
                        )}>
                          <Zap strokeWidth={1.5} size={20} />
                        </ContainerInstrument>
                        <ContainerInstrument>
                          <HeadingInstrument level={3} className="text-[15px] font-light tracking-[0.1em] uppercase">
                            <VoiceglotText  translationKey="pricing.live_regie.title" defaultText="Live Regie (Optioneel)" />
                          </HeadingInstrument>
                          <TextInstrument className="text-[15px] text-va-black/40 font-light">
                            <VoiceglotText  translationKey="pricing.live_regie.subtitle" defaultText="Regisseer de stem live via Zoom of Teams." />
                          </TextInstrument>
                        </ContainerInstrument>
                      </ContainerInstrument>
                      <ContainerInstrument className="text-right">
                        <TextInstrument className="text-lg font-extralight text-va-black">{(pricingConfig.liveSessionSurcharge / 100).toFixed(0)}</TextInstrument>
                      </ContainerInstrument>
                    </ContainerInstrument>
                    ...
                    <ButtonInstrument 
                      ...
                      className={cn(
                        ...
                        liveSession ? "border-primary bg-primary/5" : "border-black/5 bg-va-off-white/30 hover:border-black/10"
                      )}
                    >
                      <div className="flex items-center gap-4">
                        <div className={cn("w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all", liveSession ? "bg-primary border-primary text-white" : "border-black/10 bg-white")}>
                          {liveSession && <Check strokeWidth={1.5} size={14} />}
                        </div>
                        <TextInstrument className={cn("text-[15px] font-light tracking-tight", liveSession ? "text-va-black" : "text-va-black/40")}>
                          <VoiceglotText translationKey="pricing.live_regie.toggle" defaultText="Live Regie toevoegen" />
                        </TextInstrument>
                      </div>
                      <div className={cn("text-[13px] font-light tracking-[0.2em] uppercase", liveSession ? "text-primary" : "text-va-black/20")}>+ {SlimmeKassa.format(pricingConfig.liveSessionSurcharge / 100)}</div>
                    </ButtonInstrument>
                  </ContainerInstrument>
                );
              })()}
            ...
            <ContainerInstrument className="lg:w-[380px] bg-va-black text-white p-8 lg:p-12 flex flex-col justify-center text-center relative overflow-hidden">
              <ContainerInstrument className="relative z-10 space-y-8">
                ...
                {state.usage === 'paid' && mode !== 'widget' ? (
                  <div className="space-y-4">
                    <TextInstrument className="text-white/20 text-[11px] font-light tracking-[0.2em] uppercase">Tarieven variëren</TextInstrument>
                    <div className="text-2xl font-extralight tracking-tight text-white leading-tight">
                      Bekijk de individuele tarieven per stemacteur in de <span className="text-primary font-light italic">Live Ratecard</span> hieronder.
                    </div>
                  </div>
                ) : (
                  <ContainerInstrument className="space-y-2">
                    <TextInstrument className="text-6xl font-extralight tracking-tighter text-primary">
                      {pricing.formatted}
                    </TextInstrument>
                    <ContainerInstrument className="space-y-1">
                      <TextInstrument className="text-[15px] font-light text-white/20 uppercase tracking-[0.1em]">
                        <VoiceglotText  translationKey="common.excl_vat" defaultText="Exclusief BTW" />
                      </TextInstrument>
                      <TextInstrument className="text-[15px] font-light text-white/40 tracking-[0.2em] uppercase">
                ...
                <ButtonInstrument 
                  ...
                  className={cn(
                    "va-btn-pro w-full flex items-center justify-center gap-2 group !rounded-[10px]",
                    // 🛡️ USER-MANDATE: We don't block booking if it's unavailable, 
                    // instead we let the Pricing Engine handle the price display.
                    // SlimmeKassa.getAvailabilityStatus(
                    //   actor || (filteredActors && filteredActors[0]) || {}, 
                    //   state.usage === 'paid' ? media : [], 
                    //   country

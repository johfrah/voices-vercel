...
export const EmailListItemInstrument = ({
  ...
  return (
    <div className={`relative overflow-hidden border-b border-gray-50 ${isSelected ? 'ring-1 ring-va-black/10 ring-inset z-20 bg-gray-50/50' : ''}`}>
      {/* Swipe Backgrounds */}
      <motion.div 
        style={{ background }}
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center text-[15px] font-light relative shadow-sm overflow-hidden ${isSemanticResult ? 'bg-va-black text-white' : 'bg-gray-100 text-gray-500'}`}>
            {avatarUrl ? (
              ...
              <div className="relative w-full h-full">
                <Image 
                  ...
                  unoptimized
                />
                <div className="w-full h-full flex items-center justify-center bg-gray-100 text-gray-500 absolute inset-0 z-0">
                  {initial}
                </div>
              </div>
            ) : isSemanticResult ? (
            ...
            {intent === 'quote_request' && (
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 border-2 border-white rounded-full z-20" />
            )}
            {intent === 'demo_submission' && (
              <div className="absolute -top-1 -right-1 w-3 h-3 bg-blue-500 border-2 border-white rounded-full z-20" />
            )}
          </div>
          ...
          <div className="flex-grow min-w-0">
            <div className="flex justify-between items-center mb-0.5">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {personaBadge && (
                  <span className={`text-[15px] font-light tracking-widest px-1.5 py-0.5 rounded flex-shrink-0 ${personaBadge.color}`}>
                    {personaBadge.label}
                  </span>
                )}
                {intent && !personaBadge && (
                  <span className={`text-[15px] font-light tracking-widest px-1.5 py-0.5 rounded flex-shrink-0 ${
                ...
                {isSemanticResult && similarity && (
                  <span className="text-[15px] font-light tracking-widest px-1.5 py-0.5 rounded bg-va-black text-white flex-shrink-0">

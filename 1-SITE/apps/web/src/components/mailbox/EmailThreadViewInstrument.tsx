...
export const EmailThreadViewInstrument = ({ thread, actorId, onClose, onReply }: EmailThreadViewProps) => {
  ...
  return (
    <ContainerInstrument className="va-thread-view max-w-4xl mx-auto relative">
      {/* Thread Header */}
      <ContainerInstrument className="px-4 py-4 md:px-6 md:py-6 border-b border-gray-100">
        <ContainerInstrument className="flex items-start justify-between mb-2">
          ...
          <ButtonInstrument onClick={onClose} className="p-1.5 md:p-2 hover:bg-black/5 rounded-[20px] transition-colors">
        ...
        <ContainerInstrument className="flex items-center gap-3 md:gap-4">
          <ContainerInstrument className="flex items-center gap-1.5 bg-pink-50 text-pink-600 px-2 py-0.5 rounded-md text-[15px] font-light tracking-widest border border-pink-100">
            ...
            <VoiceglotText  translationKey="mailbox.secure_thread" defaultText="Secure Thread" />
          </ContainerInstrument>
          <TextInstrument as="span" className="text-[15px] font-light tracking-widest text-gray-400">
            {thread.messages.length} <VoiceglotText  translationKey="mailbox.messages_count" defaultText="berichten" />
          </TextInstrument>
        </ContainerInstrument>
      </ContainerInstrument>

      {/* Messages List */}
      <ContainerInstrument className="p-4 md:p-6 space-y-4 md:space-y-6">
        {thread.messages.map((message) => {
          ...
          return (
            <ContainerInstrument key={message.id} className="bg-white border border-gray-100 rounded-[32px] shadow-sm overflow-hidden">
              ...
              <ContainerInstrument className="px-4 py-4 md:px-6 md:py-5 flex items-start justify-between">
                <ContainerInstrument className="flex items-center gap-3 md:gap-4">
                  ...
                  <ContainerInstrument className="flex flex-col">
                    <ContainerInstrument className="flex items-center gap-2 flex-wrap">
                      ...
                      <TextInstrument as="span" className="text-[15px] text-gray-400 font-light">
                        <VoiceglotText  translationKey="mailbox.to_johfrah" defaultText="to 'Johfrah Lefebvre'" />
                      </TextInstrument>
                    </ContainerInstrument>
              ...
              <ContainerInstrument className="px-4 pb-2 md:px-6 flex gap-2">
                <ButtonInstrument className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-600 rounded-[20px] text-[15px] font-light tracking-widest border border-blue-100/50 hover:bg-blue-100 transition-all">
                  ...
                  <VoiceglotText  translationKey="mailbox.translate_en" defaultText="Translate to English" />
                </ButtonInstrument>
              ...
              {/* Message Body */}
              <ContainerInstrument 
                className="px-4 py-4 md:px-6 md:py-6 text-[15px] text-gray-800 leading-relaxed prose prose-sm max-w-none font-light"
                dangerouslySetInnerHTML={{ __html: message.htmlBody }} 
              />

              {/* Attachments (Spark Style Cards) */}
              {message.attachments && message.attachments.length > 0 && (
                <ContainerInstrument className="px-4 pb-4 md:px-6 md:pb-6 mt-4 border-t border-gray-50 pt-4 md:pt-6">
                  <ContainerInstrument className="flex items-center justify-between mb-4">
                    <ContainerInstrument className="flex items-center gap-2 text-gray-400">
                      ...
                      <TextInstrument className="text-[15px] font-light tracking-widest">{message.attachments.length} <VoiceglotText  translationKey="mailbox.attachments" defaultText="attachments" /></TextInstrument>
                    </ContainerInstrument>
                    <ButtonInstrument className="text-[15px] font-light text-blue-600 hover:underline tracking-widest bg-transparent p-0">
                      <VoiceglotText  translationKey="mailbox.save_all" defaultText="Save all" /> {(message.attachments.reduce((sum, a) => sum + a.size, 0) / 1024).toFixed(0)} KB
                    </ButtonInstrument>
                  </ContainerInstrument>
                  
                  <ContainerInstrument className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
                    {message.attachments.map((att) => (
                      ...
                      >
                          <ContainerInstrument className={`aspect-square rounded-2xl border flex flex-col items-center justify-center gap-2 md:gap-3 transition-all relative overflow-hidden ${focusedAttachmentId === att.id ? 'bg-gray-50 border-va-black/20 shadow-md' : 'bg-white border-gray-100 shadow-sm'}`}>
                            <ContainerInstrument className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-red-50 text-red-500 flex items-center justify-center">
                            ...
                            <ContainerInstrument className="text-center px-2">
                              <TextInstrument className="text-[15px] font-light text-gray-900 truncate w-full max-w-[100px]">{att.filename}</TextInstrument>
                              <TextInstrument className="text-[15px] font-light text-gray-400 ">{(att.size / 1024).toFixed(0)} KB</TextInstrument>
                            </ContainerInstrument>
              ...
              <ContainerInstrument className="px-4 py-4 md:px-6 md:py-6 bg-gray-50/30 border-t border-gray-50 flex items-center justify-between">
                <ContainerInstrument className="flex gap-2">
                  ...
                  {['Interested', 'Not interested', 'Thanks'].map((reply) => (
                    <ButtonInstrument key={reply} className="px-3 py-1 md:px-4 md:py-1.5 bg-white border border-gray-200 rounded-full text-[15px] font-light text-gray-600 hover:border-va-black transition-all shadow-sm">
                      {reply}
                    </ButtonInstrument>
                  ))}
                </ContainerInstrument>
                <ContainerInstrument className="flex gap-3 md:gap-4">
                  <ButtonInstrument onClick={() => onReply(message)} className="text-gray-300 hover:text-va-black transition-colors bg-transparent p-0">
                    <Reply strokeWidth={1.5} size={18} />
                  </ButtonInstrument>
                  <ButtonInstrument className="text-gray-300 hover:text-va-black transition-colors bg-transparent p-0">
      ...
      <AnimatePresence>
        {spotlightFile && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[10000] bg-va-black/90 backdrop-blur-md flex items-center justify-center p-4 md:p-8"
            onClick={() => setSpotlightFile(null)}
          >
            <button className="absolute top-4 right-4 md:top-8 md:right-8 text-white/50 hover:text-white transition-colors" onClick={() => setSpotlightFile(null)}>
            ...
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="max-w-5xl w-full max-h-full flex flex-col items-center gap-4 md:gap-6"
              onClick={e => e.stopPropagation()}
            >
              <ContainerInstrument className="w-full flex justify-between items-center text-white/80">
                <ContainerInstrument className="flex flex-col">
                  <TextInstrument className="text-xl md:text-2xl font-light">{spotlightFile.filename}</TextInstrument>
                ...
                <a 
                  ...
                  className="bg-white text-va-black px-4 py-2 md:px-6 md:py-3 rounded-2xl font-light tracking-widest flex items-center gap-2 hover:bg-primary hover:text-white transition-all"
                >
                  <Download strokeWidth={1.5} size={18} />
                  <VoiceglotText  translationKey="common.download" defaultText="Downloaden" />
                </a>
              </ContainerInstrument>
              ...
              <ContainerInstrument className="w-full aspect-video bg-black/40 rounded-[32px] overflow-hidden flex items-center justify-center border border-white/10 shadow-2xl">
                {isVideo(spotlightFile.filename) ? (
                  ...
                  <ContainerInstrument className="flex flex-col items-center gap-6 md:gap-8 w-full p-8 md:p-12">
                    <ContainerInstrument className="w-20 h-20 md:w-24 md:h-24 bg-white/10 rounded-full flex items-center justify-center animate-pulse">
                  ...
                  <ContainerInstrument className="text-center space-y-4">
                    ...
                    <TextInstrument className="text-white/50 font-light"><VoiceglotText  translationKey="mailbox.preview_unavailable" defaultText="Voorvertoning niet beschikbaar voor dit bestandstype." /></TextInstrument>
                  </ContainerInstrument>
                )}
              </ContainerInstrument>
            </motion.div>

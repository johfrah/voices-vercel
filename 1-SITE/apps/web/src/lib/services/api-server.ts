    ...
    try {
      ...
      try {
        ...
        if (language || lang) {
          ...
          } else if (targetLang === 'all' || targetLang === '') {
            // 🛡️ CHRIS-PROTOCOL: 'all' or empty means no language filter, show everything live
            console.log(' [getActors] Language filter is disabled, fetching all live actors');
          } else {

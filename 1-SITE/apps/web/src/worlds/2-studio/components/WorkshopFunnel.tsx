...
const FunnelStep: React.FC<FunnelStepProps> = ({ label, value, subLabel, icon, color, onClick }) => (
  <ContainerInstrument 
    onClick={onClick}
    className="flex flex-col gap-2 p-6 rounded-[32px] bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/20 transition-all cursor-pointer group active:scale-95"
  >
    ...
    <TextInstrument className="text-3xl font-black tracking-tighter text-white">{value}</TextInstrument>
...
export const WorkshopFunnel: React.FC<{ data: any }> = ({ data }) => {
  ...
  return (
    <ContainerInstrument className="space-y-8">
      {/* Funnel Visualizer */}
      <ContainerInstrument className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <FunnelStep 
      ...
      <ContainerInstrument 
        ...
        className="p-10 rounded-[40px] bg-gradient-to-br from-primary/20 to-secondary/20 border border-primary/10 hover:border-primary/30 transition-all cursor-pointer group shadow-aura active:scale-[0.99]"
      >
        <ContainerInstrument className="flex justify-between items-start">
          <ContainerInstrument>
            ...
            <TextInstrument className="text-6xl font-black tracking-tighter text-va-black group-hover:text-primary transition-colors duration-500">68.4%</TextInstrument>
        ...
        <ContainerInstrument className="mt-10 h-3 w-full bg-va-black/5 rounded-full overflow-hidden p-[2px]">
          <ContainerInstrument 
